...
def nearest_neighbour(unitary: np.ndarray, tree: VPTree, precision: float):
    # is there a member of the set with a distance precision of the given unitary?
    if tree is None:
        return False, None
    if tree.p is None:
        return False, None
...
def approx(gate_set: Dict[str, np.ndarray], unitary: np.ndarray, max_length: int, precision: float) -> Iterable:
    ...
    for i in range(math.ceil(max_length / 2)):
        S = {k1 + k2: v1 @ v2 for k1, v1 in gate_set.items() for k2, v2 in S.items()}
        for k, v in S.items():
            success, w = nearest_neighbour(v.transpose().conjugate() @ unitary, VPTree(list(S.items())), precision)
            if success:
                return k, v @ w[1]
            # Not quite sure why the for loop doesn't take care of it, but she does the above again for S_(i+1)
